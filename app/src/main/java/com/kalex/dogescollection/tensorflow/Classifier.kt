...
class Classifier(tfLiteModel: MappedByteBuffer, private val labels: List<String>) {
     ...
     */
    fun recognizeImage(bitmap: Bitmap): List<DogRecognitionConfidence> {
     ...
     */
    private fun getTopKProbability(labelProb: Map<String, Float>): List<DogRecognitionConfidence> {
        // Find the best classifications.
        val priorityQueue =
            PriorityQueue(MAX_RECOGNITION_DOG_RESULTS) { lhs: DogRecognitionConfidence, rhs: DogRecognitionConfidence ->
                (rhs.confidence).compareTo(lhs.confidence)
            }

        for ((key, value) in labelProb) {
            priorityQueue.add(DogRecognitionConfidence(key, value * 100.0f))
        }

        val recognitions = mutableListOf<DogRecognitionConfidence>()
